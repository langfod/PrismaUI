...
namespace PrismaUI::WebGL {
    ...
    // Prevent leak if JS GC collects the context object before the view is destroyed.
    // Both finalize and ViewManager cleanup run on the ultralight thread, so no race.
    static void WebGLContextFinalize(JSObjectRef obj) {
        auto* ctx = static_cast<ANGLEContext*>(JSObjectGetPrivate(obj));
        if (!ctx) return;
        // Check if context is still alive (not already destroyed by ViewManager)
        auto active = GetActiveContexts();
        if (std::find(active.begin(), active.end(), ctx) != active.end()) {
            // Clear the PrismaView's webglContext atomic so the render thread stops using it
            {
                std::shared_lock lock(Core::viewsMutex);
                for (auto& [id, view] : Core::views) {
                    if (view->webglContext.load(std::memory_order_acquire) == ctx) {
                        view->webglContext.store(nullptr, std::memory_order_release);
                        break;
                    }
                }
            }
            DestroyWebGLContext(ctx);
            logger::info("[WebGL] GC finalize: destroyed orphaned ANGLEContext");
        }
    ...
    JSClassRef GetWebGLContextClass() {
        static JSClassRef cls = []() {
            JSClassDefinition
                classDef{};  // Zero-init (avoids kJSClassDefinitionEmpty data import that breaks /DELAYLOAD)
            classDef.className = "WebGLRenderingContext";
            classDef.staticFunctions = kWebGLFunctions;
            classDef.finalize = WebGLContextFinalize;
    ...
    static JSValueRef JS_CreateWebGLContext(JSContextRef ctx, JSObjectRef function,
                                            [[maybe_unused]] JSObjectRef thisObject, size_t argc,
    ...
    // =========================================================================
    static JSValueRef JS_UpdateWebGLContext(JSContextRef ctx, [[maybe_unused]] JSObjectRef function,
                                            [[maybe_unused]] JSObjectRef thisObject, size_t argc,
                                            const JSValueRef argv[], [[maybe_unused]] JSValueRef* exc) {
        if (argc < 6) {
        ...
        if (std::isnan(wD) || std::isnan(hD) || wD < 0 || hD < 0 || wD > 16384 || hD > 16384) {
        ...
        if (!loggedFirstUpdate) {
            logger::info(
                "[WebGL-DBG] JS_UpdateWebGLContext: first update call — pos=({},{}) bufSize={}x{} displaySize={}x{} "
                "visible={}",
                canvasX, canvasY, width, height, displayW, displayH, visible);
            loggedFirstUpdate = true;
        }

        if (width > 0 && height > 0 && (width != angleCtx->canvasWidth || height != angleCtx->canvasHeight)) {
            ID3D11Device* device = PrismaUI::Core::d3dDevice;
            if (device) {

...
namespace PrismaUI::WebGL {
    ...
    JSValueRef GL_getParameter(JSContextRef ctx, JSObjectRef, JSObjectRef thisObject, size_t argc,
        ...
        switch (pname) {
            ...
            case GL_MAX_VIEWPORT_DIMS: {
                ...
                JSValueRef vals[2] = {JSValueMakeNumber(ctx, dims[0]), JSValueMakeNumber(ctx, dims[1])};
            ...
            case GL_DEPTH_RANGE: {
                ...
                JSValueRef vals[2] = {JSValueMakeNumber(ctx, range[0]), JSValueMakeNumber(ctx, range[1])};
                return JSObjectMakeArray(ctx, 2, vals, nullptr);
            }
            // Boolean arrays
    ...
    JSValueRef GL_getExtension(JSContextRef ctx, JSObjectRef, JSObjectRef thisObject, size_t argc,
        ...
        if (name == "WEBGL_multi_draw") {
            // Methods not yet implemented — don't advertise to avoid crashes
            return JSValueMakeNull(ctx);
        }
        ...
        if (name == "WEBGL_multisampled_render_to_texture") {
            // Method not yet implemented — don't advertise to avoid crashes
            return JSValueMakeNull(ctx);
        }
    ...
    JSValueRef GL_getActiveUniform(JSContextRef ctx, JSObjectRef, JSObjectRef thisObject, size_t argc,
        ...
        GLint size = 0;
        GLenum type = 0;
        glGetActiveUniform(prog, index, maxLen, &len, &size, &type, name.data());
        JSObjectRef obj = JSObjectMake(ctx, nullptr, nullptr);
        JSStringRef sizeKey = JSStringCreateWithUTF8CString("size");
        JSStringRef typeKey = JSStringCreateWithUTF8CString("type");
        JSStringRef nameKey = JSStringCreateWithUTF8CString("name");
    ...
    JSValueRef GL_isEnabled(JSContextRef ctx, JSObjectRef, JSObjectRef thisObject, size_t argc, const JSValueRef argv[],
        ...
        return JSValueMakeBoolean(ctx, glIsEnabled(static_cast<GLenum>(JSValueToNumber(ctx, argv[0], nullptr))));
    }
    ...
    JSValueRef GL_getActiveUniformBlockParameter(JSContextRef ctx, JSObjectRef, JSObjectRef thisObject, size_t argc,
        ...
        switch (pname) {
            ...
            case GL_UNIFORM_BLOCK_ACTIVE_UNIFORM_INDICES: {
                ...
                std::vector<JSValueRef> vals(count);
                for (GLint i = 0; i < count; i++) vals[i] = JSValueMakeNumber(ctx, static_cast<double>(indices[i]));
                return JSObjectMakeArray(ctx, static_cast<size_t>(count), vals.data(), nullptr);
            }
            default: {
                GLint val = 0;
    ...
    JSValueRef GL_getUniformIndices(JSContextRef ctx, JSObjectRef, JSObjectRef thisObject, size_t argc,
        ...
        std::vector<JSValueRef> vals(count);
        for (GLsizei i = 0; i < count; i++) vals[i] = JSValueMakeNumber(ctx, static_cast<double>(indices[i]));
        return JSObjectMakeArray(ctx, static_cast<size_t>(count), vals.data(), nullptr);
    }
    ...
    JSValueRef GL_getActiveUniforms(JSContextRef ctx, JSObjectRef, JSObjectRef thisObject, size_t argc,
        ...
        std::vector<JSValueRef> vals(count);
        for (GLsizei i = 0; i < count; i++) vals[i] = JSValueMakeNumber(ctx, static_cast<double>(params[i]));
        return JSObjectMakeArray(ctx, static_cast<size_t>(count), vals.data(), nullptr);
    }
